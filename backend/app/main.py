...
async def lifespan(app: FastAPI):
    ...
    try:
        async with httpx.AsyncClient() as client:
            resp = await client.get(f"{settings.ollama_base_url.rstrip('/')}/api/tags", timeout=5.0)
            if resp.status_code == 200:
                ollama_ok = True
                logger.info("Successfully reached Ollama local server.")
    except Exception as e:
        logger.warning(f"Ollama local server not reached during startup: {e}. (Ensure it is running at {settings.ollama_base_url})")
...
async def ingest_file(
    collection_type: str = Form(..., description="Target database collection: 'public' or 'papers'"),
...
async def delete_document(
    ...
    return {"message": f"Successfully deleted document '{source}' from {collection_type} collection."}
