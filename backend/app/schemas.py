...
class ArxivIngestRequest(BaseModel):
    arxiv_id: str = Field(
        ..., description="Standard arXiv identifier (e.g. 2305.16300 or 1706.03762)"
    )
    collection_type: str = Field(
        default="papers", description="Target database collection (typically 'papers')"
    )
...
class QueryRequest(BaseModel):
    ...
    strategy: str = Field(
        default="baseline",
        description="Retrieval strategy: 'baseline', 'hyde', 'multi_query', or 'flare'",
    )
    limit: int = Field(
